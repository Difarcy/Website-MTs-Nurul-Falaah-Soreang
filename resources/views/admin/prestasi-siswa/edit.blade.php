...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Prestasi Siswa</h1>
                <p class="text-sm text-slate-500 mt-1">Ubah informasi prestasi siswa</p>
            </div>
        </div>

        <form action="{{ route('admin.prestasi-siswa.update', $prestasiSiswa) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="border-2 border-gray-200 rounded-xl p-6">
                ...
                <div class="max-w-md">
                    <img src="{{ $prestasiSiswa->gambar ? asset('storage/' . $prestasiSiswa->gambar) : asset('img/default-backgrounds.png') }}" alt="Preview" class="w-full rounded-lg border border-gray-200">
                </div>
            </div>
            ...
            <!-- Informasi Prestasi -->
            <div class="space-y-4">
                <h3 class="text-lg font-semibold text-slate-900">Informasi Prestasi</h3>
                
                <div>
                    <label for="judul" class="block text-sm font-semibold text-slate-700 mb-2">
                        Judul Prestasi *
                    </label>
                    <input type="text" name="judul" id="judul" value="{{ old('judul', $prestasiSiswa->judul) }}" required placeholder="Masukkan judul prestasi" class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $prestasiSiswa->kategori) }}" placeholder="Masukkan kategori prestasi" class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_prestasi" id="tanggal_prestasi" value="{{ old('tanggal_prestasi', $prestasiSiswa->tanggal_prestasi?->format('Y-m-d')) }}" class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Masukkan deskripsi prestasi" class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">{{ old('deskripsi', $prestasiSiswa->deskripsi) }}</textarea>
            ...
            <details class="border border-gray-200 dark:border-slate-700 rounded-lg">
                ...
                <div class="px-4 pb-4 pt-2 space-y-4 border-t border-gray-200">
                    <div>
                        ...
                        <input type="number" name="urutan" id="urutan" value="{{ old('urutan', $prestasiSiswa->urutan) }}" min="0" placeholder="0" class="w-full border-2 border-gray-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-green-600 focus:border-green-600">
                    ...
                    <div>
                        <label for="is_active" class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $prestasiSiswa->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-green-600 focus:ring-green-600">
