...
@section('content')
    <div class="space-y-6">
        ...
        <form action="{{ route('admin.info-sekolah.update', $infoSekolah) }}" method="POST" class="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="key" id="key" value="{{ old('key', $infoSekolah->key) }}" required placeholder="Masukkan key (contoh: nama_sekolah)" class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600" readonly>
                    <p class="text-xs text-slate-500 mt-1">Key tidak bisa diubah setelah dibuat</p>
                ...
                <div>
                    ...
                    <input type="text" name="label" id="label" value="{{ old('label', $infoSekolah->label) }}" required placeholder="Masukkan label" class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">
                ...
                <div>
                    ...
                    <select name="type" id="type" required class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">
                        <option value="text" {{ old('type', $infoSekolah->type) == 'text' ? 'selected' : '' }}>Text (Satu Baris)</option>
                        <option value="textarea" {{ old('type', $infoSekolah->type) == 'textarea' ? 'selected' : '' }}>Textarea (Banyak Baris)</option>
                        <option value="image" {{ old('type', $infoSekolah->type) == 'image' ? 'selected' : '' }}>Image (Gambar)</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea name="value" id="value" rows="4" required placeholder="Masukkan isi informasi..." class="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-green-600 focus:border-green-600">{{ old('value', $infoSekolah->value) }}</textarea>
            ...
            <details class="border border-gray-200 dark:border-slate-700 rounded-lg">
                ...
                <div class="px-4 pb-4 pt-2 space-y-4 border-t border-gray-200">
                    <div>
                        ...
                        <input type="number" name="urutan" id="urutan" value="{{ old('urutan', $infoSekolah->urutan) }}" min="0" placeholder="0" class="w-full border-2 border-gray-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-green-600 focus:border-green-600">
