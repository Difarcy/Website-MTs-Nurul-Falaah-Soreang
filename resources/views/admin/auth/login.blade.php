    ...
    <div class="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-8 space-y-6">
        <div class="text-center space-y-3">
            <div class="flex items-center justify-center">
                <div class="flex items-center justify-center w-24 h-24 bg-white dark:bg-slate-900 rounded-xl">
                    <svg class="w-14 h-14 text-slate-900 dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                <h1 class="text-4xl font-bold text-slate-900 tracking-tight">Panel Admin</h1>
                <p class="text-base text-slate-500 mt-1 font-medium text-center">Masuk untuk mengelola konten website</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.login.attempt') }}" method="POST" class="space-y-4">
            @csrf
            <div class="space-y-1">
                <label for="admin-login" class="text-base font-semibold text-slate-700">Username</label>
                <input
                    id="admin-login"
                    type="text"
                    name="login"
