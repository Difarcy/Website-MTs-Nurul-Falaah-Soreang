...
<div class="flex flex-col gap-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Publikasi</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola berita dan artikel di website.</p>
        </div>
        <div class="flex items-center gap-3">
            <!-- Tombol Tambah (hanya tampil jika ada data) -->
            @if($posts->total() > 0)
            <a href="{{ route('admin.berita.create', request()->query()) }}" class="inline-flex items-center justify-center px-6 py-2.5 text-sm font-semibold text-white bg-green-700 rounded-lg hover:bg-green-800 min-w-[120px]">
                + Tambah
            </a>
            @endif
        </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex items-center gap-3 flex-1 flex-wrap">
            <!-- Pencarian -->
            <form method="GET" action="{{ route('admin.publikasi.index') }}" id="search-form" class="flex-1 max-w-md">
                <div class="relative">
                    <input type="text" name="q" id="search-input" value="{{ request('q') }}" placeholder="Cari judul atau ringkasan..." class="w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg px-4 py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-green-600 focus:border-green-600" onkeypress="if(event.key === 'Enter') { event.preventDefault(); submitSearch(); }" oninput="autoSearch()">
    ...
    <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 overflow-hidden" id="posts-container">
        @php
        $currentView = request('view', $viewPreference);
        @endphp
        @if($currentView === 'grid')
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 overflow-hidden hover:shadow-lg transition-shadow publikasi-grid-item">
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route(($post->type === 'artikel' ? 'admin.artikel.edit' : 'admin.berita.edit'), array_merge(['post' => $post], request()->query())) }}" class="flex-1 text-center px-3.5 py-1.5 text-xs font-bold text-white bg-green-700 rounded-lg hover:bg-green-800 whitespace-nowrap publikasi-btn-edit">Ubah</a>
                        <form action="{{ route(($post->type === 'artikel' ? 'admin.artikel.destroy' : 'admin.berita.destroy'), $post) }}" method="POST" class="flex-1 delete-form">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="w-full text-center px-3.5 py-1.5 text-xs font-bold text-white bg-red-700 rounded-lg hover:bg-red-800 whitespace-nowrap delete-btn publikasi-btn-delete" data-title="{{ $post->title }}">Hapus</button>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-full">
                <div class="text-center py-12">
                    ...
                    <div class="text-center">
                        <a href="{{ route('admin.publikasi.index') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 dark:bg-slate-700 text-white font-bold rounded-lg hover:bg-slate-800 dark:hover:bg-slate-600">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <h3 class="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2 text-center">Tidak Ditemukan Konten</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6 text-center">
                        @if(request('type'))
                        ...
                        Belum ada konten dengan status "<strong>Terpublikasi</strong>"
                        @elseif(request('status') === 'draft')
                        Belum ada konten dengan status "<strong>Draft</strong>"
                        @elseif(request('status') === 'unpublished')
                        Belum ada konten dengan status "<strong>Nonaktif</strong>"
                        @endif
                        @endif
                    </p>
                    @if(!request('status'))
                    ...
                    <div class="flex items-center justify-center gap-3">
                        <a href="{{ route('admin.berita.create', request()->query()) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-green-700 text-white font-bold rounded-lg hover:bg-green-800">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            Tambah Berita
                        </a>
                        <a href="{{ route('admin.artikel.create', request()->query()) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-700 text-white font-bold rounded-lg hover:bg-blue-800">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            Tambah Artikel
                    ...
                    <h3 class="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2 text-center">Belum Ada Konten</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6 text-center">Mulai dengan menambahkan berita atau artikel baru</p>
                    <div class="flex items-center justify-center gap-3">
                        <a href="{{ route('admin.berita.create', request()->query()) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-green-700 text-white font-bold rounded-lg hover:bg-green-800">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                    ...
                    <tr class="hover:bg-slate-200 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-right align-bottom">
                            <div class="flex flex-col items-end gap-2">
                                <a href="{{ route(($post->type === 'artikel' ? 'admin.artikel.edit' : 'admin.berita.edit'), array_merge(['post' => $post], request()->query())) }}" class="w-20 text-center px-3.5 py-1.5 text-xs font-bold text-white bg-green-700 rounded-lg hover:bg-green-800 whitespace-nowrap" style="font-size: 0.8rem;">Ubah</a>
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-12">
                            <div class="text-center">
                                ...
                                {{-- Hanya tampilkan tombol jika tidak ada filter status --}}
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('admin.berita.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-green-700 text-white font-bold rounded-lg hover:bg-green-800">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                        </svg>
                                        Tambah Berita
                                    </a>
                                    <a href="{{ route('admin.artikel.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-700 text-white font-bold rounded-lg hover:bg-blue-800">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                        </svg>
                                        Tambah Artikel
...
<script>
    ...
    const viewStorageKey = 'publikasi_view_preference';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!urlView && savedView && ['table', 'grid'].includes(savedView)) {
            url.searchParams.set('view', savedView);
            window.history.replaceState({}, '', url.toString());
        }

        if (urlView && ['table', 'grid'].includes(urlView)) {
            localStorage.setItem(viewStorageKey, urlView);
        }
    });
    ...
    function submitSearchAjax() {
        ...
        // Fetch dengan AJAX
        const url = '{{ route("admin.publikasi.index") }}?' + params.toString();
    ...
    function applyFilter() {
        ...
        @if(request('sort'))
        url.searchParams.set('sort', '{{ request('
            sort ') }}');
        @endif
        @if(request('view'))
    ...
    function applySort() {
        ...
        @endif
        @if(request('status'))
        url.searchParams.set('status', '{{ request('
            status ') }}');
        @endif
        @if(request('type'))
...
<div id="delete-modal" class="hidden fixed inset-0 bg-black/30 dark:bg-black/50 z-50 items-center justify-center">
